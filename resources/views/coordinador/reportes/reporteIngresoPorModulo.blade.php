...
<div class="panel panel-primary">
	<div class="panel-heading">
		<h3 class="panel-title">Reporte de ingresos por sección:</h3>
	</div>
	...
	<div class="panel panel-primary">
		<div class="panel-heading  text-right">
			...
			onclick="obtenerReporteIngresoPorModulo()">Descargar Reporte
			</button>
		</div>
		<div class="panel-body">
	...
	<script>
		...
		function generar() {
			...
			if (seccion.val()){
				var data = {'fecha_desde': fechaDesde.val(), 'fecha_hasta': fechaHasta.val(), 'hora_desde': horaDesde.val(), 'hora_hasta': horaHasta.val(), 'modulo': seccion.val()};
				
				$.post("{!!route('dataReporteIngresoPorModulo')!!}", data, function (result) {
					...
					if(result.estado){
						...
						$.msgbox(result.mensaje, { type: 'success' }, function(){
							...
							table[0] = $('#reporte').DataTable( {
								...
								data: result.data,
								columns: [ {data:'funcionario.tdocumento.tdoc_sigla'},{data:'funcionario.func_documento'},{data:'funcionario.func_nombres'},{data:'funcionario.func_apellidos'},{data:'acc_fecha'},{data:'acc_hora'}],
								"scrollX": true
							} );
						});
						}else{
				...
				$.msgbox("Debes seleccionar una sección para generar el reporte.", { type: 'error' }, function(){});
			}
