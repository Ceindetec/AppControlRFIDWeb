...
class reporteController extends Controller
{
    ...
    {
        $this->tituloReporte = $request->data[0]['modulo']['mod_nombre'];
        ...
        Excel::create('Reporte ingreso por modulo', function ($excel) {
            $excel->sheet('Reporte', function ($sheet) {
                $sheet->row(1, array(
                    'Sección de ingreso', $this->tituloReporte
                ));
                $sheet->fromArray($this->DataSet, null, 'A3', false);

            });
        })->store('xls');
    }

    public function obtenerReporteIngresoPorFuncionario(Request $request)
    {
        $this->funcionarioReporte = $request->data[0]['funcionario']['func_nombres'] . " " . $request->data[0]['funcionario']['func_apellidos'];
        $this->documentoReporte = $request->data[0]['funcionario']['tdocumento']['tdoc_sigla']." ".$request->data[0]['funcionario']['func_documento'];
        for ($i = 0; $i < count($request->data); $i++) {
            $this->DataSet[$i]["Sección"] = $request->data[$i]['modulo']['mod_nombre'];
            $this->DataSet[$i]["Fecha"] = $request->data[$i]['acc_fecha'];
            $this->DataSet[$i]["Hora"] = $request->data[$i]['acc_hora'];
        }
        Excel::create('Reporte ingreso por modulo', function ($excel) {
            $excel->sheet('Reporte', function ($sheet) {
                $sheet->row(1, array(
                    'Funcionario', $this->funcionarioReporte
                ));
                $sheet->row(2, array(
                    'Documento', $this->documentoReporte
                ));
                $sheet->fromArray($this->DataSet, null, 'A4', false);
    ...
    public function getreportefuncionarioxls()
    {
        $contents = Storage::disk('local')->get('Reporte ingreso por modulo.xls');
